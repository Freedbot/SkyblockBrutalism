...
namespace SkyblockBrutalism.UI
{
    ...
    internal class SkyUIState : UIState
    {
        ...
        {
            string tipsButtonText = Language.GetTextValue("Mods.SkyblockBrutalism.UI.TipsButton");
            ...
            SkyGuidePanel.Width.Set(470f, 0f);
        ...
        private bool LootSkyGuide(int writeSkyChoice)
        {
            for (int i = 0; i < 58; i++)
            {
                if (Main.LocalPlayer.inventory[i].type == ModContent.ItemType<SkyGuide>() && Main.LocalPlayer.inventory[i].ModItem is SkyGuide guide)
                {
                    if (guide.SkyChoice == 0)
                    {
                        guide.SkyChoice = writeSkyChoice;
